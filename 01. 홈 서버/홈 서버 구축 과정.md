...
- `외부IP:8080` 접속시 -> `내부IP:8080`으로 연결
- `59.6.66.238:8080` -> `192.168.0.5:8080` 

![image](https://github.com/djdjdddd/private-TIL/assets/126077503/ea000cb2-f9ef-4d17-a41d-5a9468b56d17)
...
그 후에 `도메인 포워딩` 설정을 해주시면 됩니다.
이를 통해 `구매한 DNS`로 접속해도 `공유기에 등록한 DDNS`로 매핑시켜줄 수 있습니다.
...
WORKDIR /app
# Copy only the gradle wrapper files first
COPY gradlew .
COPY gradle gradle
...
COPY --from=build /app/${JAR_FILE} help-you-move.jar
EXPOSE 8080
ENTRYPOINT ["java", "-jar", "/app/help-you-move.jar"]
```
