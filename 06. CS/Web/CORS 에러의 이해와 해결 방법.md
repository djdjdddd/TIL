...
✅ **프론트엔트 개발자 曰: "Postman으로는 요청이 정상 동작하지만, LG TxxxQ 앱에서는 CORS 에러가 발생해요"**
> 이는 **CORS가 브라우저에서만 적용되는 보안 정책이기 때문**입니다.
...
3. **브라우저의 검증 및 응답 처리**  
   ...
   - 이때, 응답 자체는 서버에서 **200 OK**로 정상적으로 내려왔을 수 있으나, 브라우저가 그것을 폐기합니다.

📌 **핵심 요점**:  
- CORS 에러는 **브라우저(클라이언트)** 에서 발생합니다.
- 해결은 반드시 서버 설정을 통해 이루어져야 합니다.
